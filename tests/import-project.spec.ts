...
test.describe('Import Project Page E2E Tests', () => {
  ...
  test.describe('8. Manual Form - Budget Plan Selection (TC-030 to TC-033, TC-424)', () => {
    ...
    test('TC-034 to TC-037 & TC-048: Budget plan selector validations', async ({ page }) => {
  ...
  test.describe('9. Manual Form - Delivery Date & Urgency Warnings (TC-034, TC-036, TC-425, TC-446, TC-049, TC-050)', () => {
    test.beforeEach(async ({ page }) => {
      await login(page, 'document_staff');
      await page.goto('/app/project-import?mode=manual');
    });

    test('TC-049: Delivery warning uses frontend urgency logic', async ({ page }) => {
      await page.locator('button#department_id').click();
      await page.getByRole('option', { name: 'สำนักงานบริหารระบบกายภาพ' }).click();
      await page.locator('button#unit_id').click();
      await page.getByRole('option', { name: 'ฝ่ายอาคารสถานที่' }).click();
    ...
    test('TC-050: Frontend urgency calculation', async ({ page }) => {
  ...
  test.describe('12. Edge Cases (TC-051, TC-052)', () => {
    test.beforeEach(async ({ page }) => {
      await login(page, 'document_staff');
    });

    test('TC-051: Duplicate-submit prevention', async ({ page }) => {
      await page.goto('/app/project-import?mode=manual');
    ...
    test('TC-052: Excel bulk import partial failure', async ({ page }) => {
